...
**Importer toutes les activités quelle que soit leur visibilité** : publiques, visibles par mes abonnés uniquement, ou visibles par moi seul. Le champ Strava `visibility` (valeurs : `everyone`, `followers_only`, `only_me`) ou `private` permet de les identifier — ne filtrer sur aucune de ces valeurs.
